...
use super::{Action, Announce, AnnounceParams, Event};
...
impl TrackerUdpClient {
    ...
    pub async fn announce(&mut self, params: &AnnounceParams) -> std::io::Result<Announce> {
        ...
        let n = udp_recv_timeout(&self.socket, &mut buffer).await?;
        let buffer = &buffer[..n];
    ...
    async fn connect(&mut self) -> std::io::Result<u64> {
...
async fn udp_recv_timeout(socket: &UdpSocket, buffer: &mut [u8]) -> std::io::Result<usize> {
    let fut = socket.recv(buffer);
    let timeout = tokio::time::sleep(DEFAULT_TIMEOUT);
    tokio::select! {
        n = fut => n,
        _ = timeout => Err(std::io::Error::new(std::io::ErrorKind::TimedOut, "timeout")),
    }
}
